...
<ul class="nav nav-tabs nav-statuses">
    @foreach($statuses as $status => $item)
        <li>
            <a href="{{ route('platform.tracks.index', compact('status')) }}" class="@if(request('status', array_key_first($statuses)) == $status) active @endif">
                {{ $item['name'] }} @if($item['count'])<span class="label label-{{ App\Models\Track::STATUS_COLORS[$status] }}">{{ $item['count'] }}</span>@endif
            </a>
        </li>
    @endforeach
</ul>
