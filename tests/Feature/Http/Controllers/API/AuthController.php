...
class AuthController extends TestCase
{
     ...
     * @return void
     */
    public function testSocial_NotAuthParams()
    {
        ...
        # перебераем все варианты и проверяем ответ
        foreach ($combinations as $item) {
            static::clearLoginAttempts();

            $this->postJson(LOGIN_SOCIAL_URI, $item)
                ->assertStatus(SymfonyResponse::HTTP_BAD_REQUEST)
                ->assertExactJson([
                    'status' => false,
                    'errors' => ['These credentials do not match our records.']
     ...
     * @return void
     */
    public function testSocial_WrongProvider()
    {
        static::clearLoginAttempts();
     ...
     * @return void
     */
    public function testSocial_Throttling()
    {
        static::clearLoginAttempts();
